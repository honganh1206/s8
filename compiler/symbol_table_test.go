...
func TestResolveNestedLocal(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			[]Symbol{
				{Name: "a", Scope: GlobalScope, Index: 0},
				{Name: "b", Scope: GlobalScope, Index: 1},
				// Must start with 0 to not be tied to other scopes
				{Name: "c", Scope: LocalScope, Index: 0},
				{Name: "d", Scope: LocalScope, Index: 1},
			},
		},
	...
	for _, tt := range tests {
		for _, sym := range tt.expectedSymbols {
			// Expect the tables to tell different scopes apart
...
func TestResolveFree(t *testing.T) {
	...
	*/
	...
	for _, tt := range tests {
		// All identifiers in the arithmetic expressions can be resolved correctly
		for _, sym := range tt.expectedSymbols {
			result, ok := tt.table.Resolve(sym.Name)
			if !ok {
				t.Errorf("name %s not resolvable", sym.Name)
		...
		// Detect and keep track of symbols identified as free variables.
		// In this case c and d should be resolved as FreeScope.
...
func TestResolveUnresolvableFree(t *testing.T) {
	// Ensure we are not automatically marking every symbol as a free variable
	global := NewSymbolTable()
	global.Define("a")
...
func TestDefineResolveBuiltins(t *testing.T) {
	// Ensure builtin functions alway resolve to a symbol in BuiltinScope
	global := NewSymbolTable()
	firstLocal := NewEnclosedSymbolTable(global)
	secondLocal := NewEnclosedSymbolTable(firstLocal)
	expected := []Symbol{
